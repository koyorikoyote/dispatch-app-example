...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkBiometricAvailability = async () => {
    if (Platform.OS === 'web') {
      // On web, check if WebAuthn is available
      setBiometricAvailable(
        typeof window !== 'undefined' &&
        !!window.PublicKeyCredential
      )
      // Check if user previously enrolled (stored flag)
      const enrolled = await secureStorage.getItem(BIOMETRIC_ENABLED_KEY)
      setBiometricEnrolled(enrolled === 'true')
    } else {
      // On native, check for biometric hardware
      try {
        ...
        setBiometricEnrolled(enabled === 'true')
      } catch {
  ...
  const loginWithBiometric = async (): Promise<boolean> => {
    try {
      ...
      if (Platform.OS === 'web') {
        // Web: Use WebAuthn discoverable credentials
        const { startAuthentication } = await import('@simplewebauthn/browser')
        const { challengeToken, options } = await authApi.webauthnLoginStart()
        const credential = await startAuthentication(options)
        const response = await authApi.webauthnLoginFinish(challengeToken, credential)
        ...
        // Native: Use expo-local-authentication + stored refresh token
        const LocalAuthentication = await import('expo-local-authentication')
        ...
        if (!storedRefreshToken) {
          setError('No stored credentials. Please log in with password first.')
          return false
        }
      ...
      const errorMessage = err.message || 'Biometric login failed'
      setError(errorMessage)
      return false
    } finally {
      setIsLoading(false)
