...
import { useMessageRepliesSSE } from "../hooks/useMessageRepliesSSE"
...
// ConversationSection uses ConversationService's "tableName" enum at the API
// layer, but ConversationService keys its event broadcasts on the shorter
// "parentType" discriminator. Bridge between the two so the SSE filter can
// match server-side.
const tableNameToParentType: Record<
    "inquiries" | "daily_record" | "interaction_records" | "complaint_details",
    "inquiry" | "dailyRecord" | "interaction" | "complaint"
> = {
    inquiries: "inquiry",
...
export function ConversationSection({ tableName, recordId }: ConversationSectionProps) {
    ...
    console.log("[chat-debug] ConversationSection render — refreshTrigger=", refreshTrigger, "recordId=", recordId)
    ...
    const loadReplies = async (silent: boolean = false) => {
        if (!silent) setIsLoading(true)
        setError(null)
        try {
            console.log("[chat-debug] ConversationSection loadReplies fetching", { tableName, recordId, silent })
            const response = await apiClient.get<{ success: boolean; data: Reply[] }>(getEndpoint())
            console.log("[chat-debug] ConversationSection loadReplies got", response?.data?.length, "replies")
    ...
    // Re-fetch this thread whenever the global RefreshContext counter bumps.
    // The notifications screen bumps it from its (proven-working) stream
    // callback, so any reply that arrives via that SSE channel will pulse
    // through here as a React state change — guaranteed to trigger this
    // effect even if a separate in-dialog SSE wouldn't.
    const loadRepliesRef = useRef(loadReplies)
    useEffect(() => {
        loadRepliesRef.current = loadReplies
    })
    useEffect(() => {
        console.log("[chat-debug] ConversationSection refresh effect, refreshTrigger=", refreshTrigger, "recordId=", recordId)
        if (refreshTrigger > 0 && recordId) {
            void loadRepliesRef.current(true)
        }
    }, [refreshTrigger, recordId])

    // Direct, low-latency patch path for the case where the dispatch user is
    // the reply author rather than the parent's staff (notifications stream
    // wouldn't fire for them then). Idempotent with the refetch above.
    useMessageRepliesSSE({
        parentType: tableNameToParentType[tableName],
        parentId: recordId,
        enabled: !!recordId,
        onReplyChanged: (f) => {
            if (f.action === "delete") {
                setReplies((prev) => prev.filter((r) => r.id !== f.replyId))
                return
            }
            const incoming = f.reply as unknown as Reply | null
            if (!incoming) return
            setReplies((prev) => {
