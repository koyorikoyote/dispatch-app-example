...
export default function SubmissionsPage() {
    ...
    // Real-time updates: when the server pushes a record-changed event for
    // one of the submission tables, trigger the existing refresh flow (which
    // re-applies filters, sorting, and pagination correctly). This is still
    // event-driven push — there is no polling interval here.
    useEffect(() => {
        const unsub = subscribeStream("record-changed", (frame) => {
            const t = frame.data.tableName
            if (
                t === "inquiries" ||
                t === "daily_record" ||
                t === "complaint_details" ||
                t === "interaction_records"
            ) {
                refreshSubmissions()
            }
        })
        return unsub
    }, [subscribeStream, refreshSubmissions])
