        ...
        const { disconnect } = connectNotificationStream(
            (event) => {
                if (event.type === "snapshot") {
                    // Initial sync on every (re)connect — just adopt the list.
                    // Do NOT triggerRefresh here, because handlePullRefresh
                    // re-calls attachStream() which causes another snapshot,
                    // which would loop indefinitely.
                    setNotifications(event.notifications ?? [])
                } else if (event.type === "update") {
                    setNotifications(event.notifications ?? [])
                    // Real change — propagate to any open dialog via
                    // RefreshContext so ConversationSection re-fetches.
                    triggerRefresh()
                }
            },
            (_err) => {
